...
const inferTagsFromTitle = (title) => {
    if (!title)
        return [];
...
export const toBackendType = (contentType) => contentType;
export const deriveDeterministicTags = (extraction) => {
...
export const deriveCategory = (extraction) => {
    if (extraction.platform === "youtube")
        return "Entertainment";
    if (extraction.platform === "reddit" || extraction.platform === "twitter")
        return "News";
    if (extraction.contentType === "document")
        return "Research";
    return "Other";
};
export const deriveTopics = (tags, category) => {
...
export const buildDeterministicDescription = (extraction) => {
    const primary = extraction.metadata.description ||
...
export const shouldUseAiSynthesis = (extraction, mode) => {
    if (mode !== "deep")
        return false;
...
export const truncateForSynthesis = (content, maxChars = 5000) => content.length <= maxChars ? content : `${content.slice(0, maxChars)}...`;
