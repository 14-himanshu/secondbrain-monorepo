...
export const assessExtractionQuality = (text, source, platform) => {
    ...
    const shortLineRatio = lines.length > 0 ? lines.filter((line) => line.split(/\s+/).filter(Boolean).length <= 4).length / lines.length : 0;
    ...
    if (issues.includes("too-short"))
        score -= 0.35;
    if (issues.includes("navigation-noise"))
        score -= 0.25;
    if (issues.includes("repeated-text"))
        score -= 0.2;
    if (issues.includes("navigation-heavy"))
        score -= 0.25;
    if (issues.includes("recommended-content"))
        score -= 0.2;
...
export const adjustConfidence = (baseConfidence, validation) => {
    const penalty = validation.issues.reduce((total, issue) => {
        if (issue === "too-short")
            return total + 0.12;
        if (issue === "navigation-noise" || issue === "navigation-heavy")
            return total + 0.15;
        if (issue === "repeated-text" || issue === "recommended-content")
            return total + 0.1;
        if (issue === "empty")
            return total + 0.5;
        return total;
...
export const deriveExtractionQuality = (validation, sourceType) => {
    if (sourceType === "protected_source")
        return "low";
    if (!validation.passed || validation.score < 0.55 || validation.wordCount < 40)
        return "low";
    if (validation.score >= 0.82 && validation.wordCount >= 120)
        return "high";
    return "medium";
};
