...
const log = (level: "info" | "warn" | "error", msg: string, meta?: any) => {
  if (meta) {
    if (level === "info") console.info(`[GOOGLE_AUTH] ${msg}`, meta);
    else if (level === "warn") console.warn(`[GOOGLE_AUTH] ${msg}`, meta);
    else console.error(`[GOOGLE_AUTH] ${msg}`, meta);
  } else {
    if (level === "info") console.info(`[GOOGLE_AUTH] ${msg}`);
    else if (level === "warn") console.warn(`[GOOGLE_AUTH] ${msg}`);
    else console.error(`[GOOGLE_AUTH] ${msg}`);
  }
};

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

export const decryptToken = (enc?: string | null) => {
  if (!enc) return null;
  try {
    return decrypt(enc);
  } catch (e) {
    log("error", "decrypt_failed", { error: (e as Error).message });
    return null;
  }
};

export const encryptToken = (plain?: string | null) => {
  if (!plain) return null;
  try {
    return encrypt(plain);
  } catch (e) {
    log("error", "encrypt_failed", { error: (e as Error).message });
    return null;
  }
};
...
export const getAccessTokenForUser = async (userId: string) => {
  ...
  const lockResult = await withDistributedLock(`google:refresh:${userId}`, 120000, async () => {
    const user = await UserModel.findById(userId).select(
      "+google.accessTokenEnc +google.refreshTokenEnc google.expiryDate google.connected"
    );
    if (!user) throw new Error("USER_NOT_FOUND");

    const google = (user as any).google || {};
    if (!google.connected) throw new Error("GOOGLE_NOT_CONNECTED");

    const accessEnc: string | undefined = google.accessTokenEnc;
    const refreshEnc: string | undefined = google.refreshTokenEnc;
    const expiryDate: Date | undefined = google.expiryDate;
    ...
    if (!refreshToken) {
      ...
      if (!accessToken) throw new Error("NO_REFRESH_OR_ACCESS_TOKEN");
      return accessToken;
    }
    ...
    if (!shouldRefresh) return accessToken as string;
  ...
  if (!lockResult.acquired) {
    ...
    if (!access) throw new Error("REFRESH_LOCKED_NO_TOKEN");
    return access as string;
  }

  // If we acquired the lock, lockResult.value is the token (string)
  return lockResult.value as unknown as string;
};
