...
(function (AIErrorCode) {
    AIErrorCode["RATE_LIMIT"] = "AI_RATE_LIMIT";
    AIErrorCode["CONTEXT_FAILURE"] = "AI_CONTEXT_FAILURE";
    AIErrorCode["SYNTHESIS_ERROR"] = "AI_SYNTHESIS_ERROR";
    AIErrorCode["EMBEDDING_FAILURE"] = "AI_EMBEDDING_FAILURE";
...
const updateMetrics = (latency, tokens = 0, error = null) => {
...
const isValidKey = (key) => !!key && !key.includes("****") && key.length > 20;
...
const withTimeout = (promise, ms, label) => {
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new AIError(AIErrorCode.TIMEOUT, `[TIMEOUT] ${label} exceeded ${ms}ms`, true)), ms));
 ...
 */
const invokeLLM = async (params, retries = 2) => {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (!isRetryable || attempt === retries) {
                ...
                throw new AIError(isRateLimit ? AIErrorCode.RATE_LIMIT : isTimeout ? AIErrorCode.TIMEOUT : AIErrorCode.SYNTHESIS_ERROR, sanitizeError(err), isRetryable);
            }
...
const trimTag = (tag) => tag.trim().toLowerCase().replace(/[^a-z0-9+#.\-_ ]+/g, " ").replace(/\s+/g, "-");
...
const buildAiMetadata = (normalizedUrl, extraction, topics, options) => {
...
const buildSynthesisDescription = (result, deterministicDescription) => {
...
const buildFallbackClassification = (normalizedUrl, extraction) => {
...
const buildManualFallbackClassification = (normalizedUrl, extraction, reason) => {
    const tags = deriveDeterministicTags(extraction);
...
const getManualContentReason = (extraction, mode) => {
    if (mode !== "deep")
        return null;
    if (extraction.sourceType === "protected_source")
        return "protected_source";
    if (!extraction.validation.passed)
        return "validation_failed";
    if (extraction.extractionQuality === "low")
        return "low_confidence_extraction";
    if (extraction.confidence < 0.75)
        return "low_confidence_extraction";
    if (extraction.wordCount < 80)
        return "insufficient_content";
    if (["metadata", "body-fallback", "unavailable"].includes(extraction.source))
        return "insufficient_content";
    return null;
};
const shouldReuseCachedSummary = (doc, normalizedLink) => Boolean(doc &&
...
export const getAiClassification = async (url, mode = "deep", context) => {
    try {
...
${truncateForSynthesis(extraction.content)}`,
        ...
        const result = parseJsonObject(llmResponse);
 ...
 */
export const createEmbedding = async (text, useCache = false) => {
...
export const processContentEmbedding = async (contentId) => {
    if (currentlyProcessing.has(contentId))
        return;
    ...
    try {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (!content || content.embeddingStatus === "completed")
                    break;
                const normalizedLink = String(content.normalizedLink || content.link || "");
                if (!normalizedLink) {
                    throw new Error("Missing link for content embedding");
                }
                const duplicateResult = await withDistributedLock(`ai:lock:url:${normalizedLink}`, 120000, async () => {
                    ...
                    if (reusable &&
                        ...
                        await ContentModel.findByIdAndUpdate(contentId, {
                            ...
                            aiMetadata: reusable.aiMetadata || content.aiMetadata,
                    ...
                    try {
                        ...
                        console.warn("[GOOGLE_AUTH][ENSURE_FAILED]", e.message);
                    }
                    const classification = await withTimeout(getAiClassification(content.link, "deep"), 45000, "ExtractAndSummarize");
                    ...
                    await ContentModel.findByIdAndUpdate(contentId, {
                        ...
                        normalizedLink: classification.normalizedLink || content.normalizedLink || content.link,
                    ...
                    const updateResult = await ContentModel.updateOne({ _id: contentId }, {
                        embedding,
                        embeddingStatus: "completed",
                        aiStatus: "completed",
                        aiProgress: 100,
 ...
 */
export const generateAiChatAnswer = async (query, context, history = []) => {
    const contextBlob = context.map((c, i) => `[Source ${i + 1}]: Title: ${c.title} | Link: ${c.link} | Type: ${c.type} | ID: ${c._id}\nSummary: ${c.description || "No summary available."}`).join("\n\n");
 ...
 */
export const generateAiChatAnswerStream = async (query, context, history = [], onChunk) => {
    try {
        if (!groq)
            throw new AIError(AIErrorCode.SYNTHESIS_ERROR, "AI service not configured.");
        const contextBlob = context.map((c, i) => `[Source ${i + 1}]: Title: ${c.title} | Link: ${c.link} | Type: ${c.type} | ID: ${c._id}\nSummary: ${c.description || "No summary available."}`).join("\n\n");
        const systemPrompt = `You are a high-fidelity "Second Brain" assistant. 
    Your primary directive is to synthesize answers using ONLY the provided context.
 ...
 */
export const generateSemanticClusters = async (contents) => {
    const clusters = [];
    ...
    for (const item of validContents) {
        ...
        const similar = validContents.filter(other => {
            if (processedIds.has(other._id.toString()))
                return false;
...
export const generateBrainIntelligence = async (userId, contents, deterministicData) => {
    if (!groq)
 ...
 */
export const getDeterministicAnalytics = async (userId) => {
    try {
