...
const extractPlainText = (richText) => richText.map((item) => String(item?.plain_text || "").trim()).filter(Boolean).join(" ");
const normalizeWhitespace = (value) => value.replace(/\s+/g, " ").trim();
const collectBlockText = (block) => {
...
const listBlockChildren = async (blockId, token, notionVersion) => {
...
export const extractNotionContent = async (target, mode = "deep") => {
    ...
    try {
        ...
        const title = normalizeWhitespace(extractPlainText(Array.isArray(titleProperty?.title) ? titleProperty.title : [])) || "Notion page";
...
//# sourceMappingURL=notion.extractor.js.map