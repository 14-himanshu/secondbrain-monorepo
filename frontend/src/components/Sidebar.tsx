  ...
  return (
    <button
      ...
      className={`group relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-left outline-none focus-visible:ring-2 focus-visible:ring-purple-400 ${
        ...
        active
          ? "bg-purple-50/50 dark:bg-purple-900/30 text-[#6f63d9] dark:text-[#8378e8] font-semibold"
    ...
    >
      ...
      <span
        className={`flex-shrink-0 transition-colors duration-200 ${
          active ? "text-[#6f63d9] dark:text-[#8378e8]" : "text-slate-400 group-hover:text-[#6f63d9]"
        }`}
      >
        {icon}
      </span>
      ...
      {/* Collapsed tooltip */}
      {isCollapsed && (
        <span className="pointer-events-none absolute left-full ml-3 z-50 whitespace-nowrap rounded-lg bg-slate-900 px-2.5 py-1.5 text-[11px] font-semibold text-white shadow-lg opacity-0 translate-x-[-6px] group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200">
          {label}
        </span>
      )}
    </button>
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="absolute top-7 -right-4 z-50 w-8 h-8 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm flex items-center justify-center text-slate-500 hover:text-[#6f63d9] dark:hover:text-[#8378e8] hover:border-purple-200 dark:hover:border-purple-500 transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] hover:scale-105"
        ...
        <div className={`flex items-center gap-3 px-4 pt-6 pb-5 flex-shrink-0 ${isCollapsed ? "justify-center px-2" : ""}`}>
          ...
          {!isCollapsed && (
            <div className="min-w-0 transition-opacity duration-200">
              <div className="text-[14px] font-bold text-slate-900 dark:text-white tracking-tight leading-none">
                Second Brain
              </div>
        ...
        <div className="flex-shrink-0 px-2">
          {!isCollapsed && (
            <div className="px-3 mb-2 text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500">
              Menu
            </div>
          )}
                ...
                if (isCollapsed) {
                  return (
                    <button
                      ...
                      className={`group relative flex justify-center items-center p-2.5 rounded-xl transition-all duration-200 ${isActive ? "bg-purple-50 text-[#6f63d9]" : "text-slate-500 hover:bg-purple-50/50 hover:text-[#6f63d9]"}`}
                      title={c.title}
                    >
                      <div className="scale-75 flex items-center justify-center">
                        {getRecentIcon(c.type)}
                      </div>
                      <span className="pointer-events-none absolute left-full ml-3 z-50 rounded-lg bg-slate-900 px-2.5 py-1.5 text-[11px] font-semibold text-white shadow-lg opacity-0 translate-x-[-6px] group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 w-[180px] line-clamp-2 whitespace-normal break-words leading-snug">
                        {c.title}
                      </span>
                    </button>
                  );
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className={`text-[12px] font-medium line-clamp-2 leading-tight whitespace-normal break-words ${isActive ? "text-[#6f63d9] dark:text-[#8378e8]" : "text-slate-700 dark:text-slate-400 group-hover:text-[#6f63d9] dark:group-hover:text-[#8378e8]"}`}>
                        {c.title}
                      </div>
                      <div className="flex items-center justify-between mt-0.5">
                        ...
                        {('createdAt' in c && c.createdAt) ? (
                          <div className="text-[9px] font-medium text-slate-400 dark:text-slate-500 shrink-0 ml-2">
              ...
              <div className="relative flex justify-center">
                ...
                {profileMenuOpen && (
                  <div className="absolute bottom-full left-0 mb-2 w-44 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-lg py-1.5 z-50">
                    <div className="px-3 py-2 border-b border-gray-50 dark:border-gray-700">
                      <div className="text-[12px] font-semibold text-slate-900 dark:text-slate-200 truncate">{userProfile.username}</div>
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-[13px] font-semibold text-slate-900 dark:text-slate-200 leading-none truncate max-w-[100px]">
                      {userProfile.username}
                    </div>
                    <div className="text-[11px] text-slate-500 mt-1 leading-none">Member</div>
                  </div>
                </div>
