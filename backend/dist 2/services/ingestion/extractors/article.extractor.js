...
const buildMetadataContent = (title, description, excerpt) => normalizeWhitespace([title, description, excerpt].filter(Boolean).join(". "));
...
export const extractArticleContent = async (target, mode = "deep") => {
    ...
    try {
        ...
        const readable = new Readability(dom.window.document.cloneNode(true)).parse();
        ...
        const inferredContentType = "post";
...
//# sourceMappingURL=article.extractor.js.map