...
app.post("/api/v1/content", userMiddleware, async (req, res) => {
    ...
    if (link && !description) {
        ...
        if (!queueResult.enqueued) {
            processContentEmbedding(content._id.toString()).catch(err => console.error("[AUTO_AI_FAILED]", err.message));
        }
    }
    res.json({
        success: true,
...
import { googleConnectController, googleSigninStart, googleCallbackController, googleStatusController, googleDisconnectController, exchangeLoginCode, } from "./controllers/google-integration.controller.js";
...
app.put("/api/v1/content", userMiddleware, async (req, res) => {
    ...
    try {
        ...
        const result = await ContentModel.updateOne({ _id: contentId, userId: req.userId }, { $set: updateData });
...
app.post("/api/brain/share", userMiddleware, async (req, res) => {
    try {
        ...
        const link = await LinkModel.findOneAndUpdate({ userId: userObjectId }, { $set: update }, {
            new: true,
            upsert: true,
        ...
        if (!link)
            throw new Error("Database failed to upsert link");
        res.json({
